...
SELECT * FROM vendas_itens2 WHERE produto_id = 1 ORDER BY data_venda;

SELECT 
  produto_id,
  unidade,
  ROUND(SUM(quantidade), 3) AS qtde_vendida,
  ROUND(SUM(quantidade * valor_unitario), 3) AS total_vendido,
  ROUND(SUM(quantidade * valor_unitario) / SUM(quantidade), 3) AS media_valor_unitario
FROM
  vendas_itens2
GROUP BY
  produto_id,
